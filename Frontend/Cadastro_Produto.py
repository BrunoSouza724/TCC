...
def Cadastro_Produto():
    ...
    if not st.session_state["modo_edicao_produto"]:
        novo_produto = st.text_input("Digite o nome do produto para cadastrar:")
        ...
        if st.button("Salvar Produto"):
            if novo_produto.strip() == "":
                ...
                con = conectar_banco()
                cursor = con.cursor()
                cursor.execute("SELECT COUNT(*) FROM Produtos WHERE LOWER(nome) = LOWER(?)", (nome_formatado,))
                existe = cursor.fetchone()[0]
                con.close()

                if existe:
                    st.error(f"⚠️ O produto '{nome_formatado}' já está cadastrado.")
                else:
                    try:
                        con = conectar_banco()
                        cursor = con.cursor()
                        cursor.execute("INSERT INTO Produtos (nome) VALUES (?)", (nome_formatado,))
                        con.commit()
                        st.success(f"✅ Produto '{nome_formatado}' cadastrado com sucesso!")
    ...
    if produtos:
        ...
        if st.session_state["modo_edicao_produto"]:
            ...
            produto_selecionado = st.selectbox("Selecione um produto para excluir:", nomes_produtos)
            ...
            if st.session_state["exibir_confirmacao"]:
                ...
                with col_msg:
                    st.warning(f"Tem certeza que deseja excluir o produto **{st.session_state['produto_para_excluir']}**?")
                with col_sim:
                    if st.button("✅ Sim", key="btn_sim"):
                        try:
                            con = conectar_banco()
                            cursor = con.cursor()
                            cursor.execute("DELETE FROM Produtos WHERE nome = ?", (st.session_state["produto_para_excluir"],))
                            con.commit()
                            st.success(f"✅ Produto '{st.session_state['produto_para_excluir']}' excluído com sucesso!")
                        except sqlite3.Error as e:
